...
public class FieldOfView : MonoBehaviour {
    ...
    void Start() {
        ...
        // Set cone colour
        mr.material.color = new Color(0f, 1f, 0f, 0.1f);
    ...
    void Update() {
        ...
        float alpha = Mathf.Atan2(positionOnScreen.y - mouseOnScreen.y, positionOnScreen.x - mouseOnScreen.x) + 2.82f;
        ...
        if (paused == false)
        {
            ...
            for (int i = numPoints / 4; i <= (numPoints / 4 + numPoints / 10); i++)
            {
                // Find x position from coordinates
                float x = radius * Mathf.Cos(alpha);
                // Find y position from coordinates
