...
public class Shoot : MonoBehaviour
{
    ...
    public static int checkpointAmmo;
    ...
    void Update()
    {
        ...
        {
            // Yellow
            ammoHUDColour.r = 1f; // same as prev
        ...
        else if (ammo == 2 || ammo == 1)
        {
            ...
            ammoHUDColour.b = 0.016f; // same as prev
        }
        else if (ammo == 0)
        {
            // Red
        ...
        if (cooldown <= 0)
        {
            if (Input.GetButton("Fire1") && paused == false)
            {
                if (ammo > 0)
                {
                    playerAudio.Shoot();
                    ...
                    rb.AddForce(shotOrigin.up * shotForce, ForceMode2D.Impulse);
